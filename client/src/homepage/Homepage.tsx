...
export const Homepage: React.FC = () => {
    ...
    useEffect(()=>{
      fetch(`https://www.devcanada.online/get-region-data?date=${'2022-12-23'}&table=region_data`)
      .then((res: Response)=>{
        if (res['status'] > 400) {
    ...
    useEffect(()=>{
      fetch(`https://www.devcanada.online/get-region-data?date=${'2022-12-23'}&table=technology_data`)
      .then((res: Response)=>{
        if (res['status'] > 400) {
          throw new Error(`${res['status']}, ${res['statusText']}`)
        }
